...
handle_extension() {
    case "${FILE_EXTENSION_LOWER}" in
        ...
        ledger)
            ledger -f "${FILE_PATH}" r --sort '-date'
            exit 1;;

        csv)
            column -s, -t < "${FILE_PATH}"  | less -#2 -N -S
            exit 1;;
...
# fi
handle_extension | head -n 100
handle_mime "${MIMETYPE}" | head -n 100
handle_fallback | head -n 100
